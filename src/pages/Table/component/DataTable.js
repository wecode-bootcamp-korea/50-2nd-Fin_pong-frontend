...
const DataTable = ({
  headers,
  flows = [], // items props 받기, default parameter 빈 배열로 설정
  ...
  //       getAbledItems(flows).map((item) => item[itemKey]),
  //     );
  ...
  const onChangeSelectAll = (e) => {
    if (e.target.checked) {
      ...
      const allItems = flows.map((item) => item[itemKey]);
      setSelection(new Set(allItems));
  ...
  const isSelectedAll = () => {
    return selection.size === getAbledItems(flows).length;
  };
